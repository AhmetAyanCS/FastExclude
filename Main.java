...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int k=0; k<sourceNode.getNeighbors().size(); k++){
			...
			System.out.println("Source = "+tempSourceNode.getID()+" Target = "+targetNode.getID());
			...
			for(int j=0; j<allPaths.size(); j++){
				System.out.print(j+" - Distance("+allPaths.get(j).getDistance()+") ");
		...
		/*ArrayList<AllPaths> allPaths = new ArrayList<AllPaths>();
		FindAllPaths FAP = new FindAllPaths(Graph,sourceNode,node10);  // Calculate all paths from source to target
		
		for(Stack<Node> path : FAP.allPaths){	// Create allPaths nodes and add to allPaths arraylist
			AllPaths newPath = new AllPaths(path, path.get(0), path.get(path.size()-1), calculateDistance(Edges, path));
			allPaths.add(newPath);
		}
